    ...
    def fill_attendance():
        ...
        try:
            ...
            connection = pymysql.connect(
                host='localhost', user='root', password='Sql@2024', db='manually_fill_attendance')
        ...
        if subb == '':
            ...
            def enter_data_DB():
                ...
                if Enrollment == '':
                    ...
                    # Generate current timestamp for Date and Time
                    ts = time.time()
                    ...
                    # Check if the Enrollment already exists in the CSV file to avoid duplicates
                    duplicate_found = False
                    try:
                        with open(r'StudentDetails\StudentDetails.csv', 'r', newline='') as csvFile:
                            reader = csv.reader(csvFile)
                            for row in reader:
                                if row[0] == Enrollment:  # Check if Enrollment number exists
                                    duplicate_found = True
                                    break
                    except Exception as e:
                        print(f"CSV file read error: {e}")

                    # If no duplicate, append the new data to the CSV
                    if not duplicate_found:
                        try:
                            row = [Enrollment, Name, Date, Time]
                            # Write to StudentDetails.csv
                            with open(r'StudentDetails\StudentDetails.csv', 'a+', newline='') as csvFile:
                                writer = csv.writer(csvFile, delimiter=',')
                                writer.writerow([Enrollment, Name, Date, Time])
                            # Write to the new CSV in the 'Manually Attendance' folder
                            manually_attendance_path = r'C:\Users\hp\Desktop\Attendance Management System using Face Recognition\Attendance\Manually Attendance'
                            manually_attendance_file = f"{manually_attendance_path}\\{DB_table_name}.csv"
                            
                            with open(manually_attendance_file, 'a+', newline='') as csvFile:
                                writer = csv.writer(csvFile, delimiter=',')
                                writer.writerow([Enrollment, Name, Date, Time])
                        except Exception as e:
                            print(f"CSV file write error: {e}")
            
                        # Notify the user that the data has been saved successfully
                        Notification.configure(text="Student details saved!", bg="SpringGreen3", width=50, font=('times', 18, 'bold'))
                        Notification.place(x=250, y=400)
                    else:
                        # Notify the user if the data already exists (duplicate)
                        Notification.configure(text="Enrollment already exists!", bg="Red", width=50, font=('times', 18, 'bold'))
                        Notification.place(x=250, y=400)
            ...
            def create_csv():
                import csv
                global cursor  # Declare cursor as global so that it can be accessed from this function that was initialized in database connection setup
                cursor.execute("select * from " + DB_table_name + ";")
                csv_name = 'C:/Users/hp/Desktop/Attendance Management System using Face Recognition/Attendance/Manually Attendance/'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(
                    r'explorer /select,"C:\Users\hp\Desktop\Attendance Management System using Face Recognition\Attendance\Manually Attendance\-------Check atttendance-------"')
...
def subjectchoose():
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    global cursor
                    connection = pymysql.connect(
                        host='localhost', user='root', password='Sql@2024', db='Face_reco_fill')
                    cursor = connection.cursor()
                except Exception as e:
                    print(e)
                ...
                root.configure(background='grey80')
                cs = 'C:/Users/hp/Desktop/Attendance Management System using Face Recognition/' + fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(
            r'explorer /select,"C:\Users\hp\Desktop\Attendance Management System using Face Recognition\Attendance\-------Check atttendance-------"')
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'trishita':
            if password == 'project2':
                win.destroy()
                import csv
                import tkinter
                root = tkinter.Tk()
                ...
                cs = 'C:/Users/hp/Desktop/Attendance Management System using Face Recognition/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
